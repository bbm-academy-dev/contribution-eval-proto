(function () {
  ...
  function init() {
    const setup = Sprint.decodeSetup(window.location.hash);
    ...
    Sprint.PRODUCTS.forEach((product) => {
      ...
      section.innerHTML = `
        <div class="product-toggle">
          ...
          <span class="weight-chip">${Sprint.percent(state.weights[product.key] || 0, 1)} веса</span>
        </div>
  ...
  function renderGlue() {
    glueMount.innerHTML = `
      <label class="field-label">Что я внёс в общую ткань спринта помимо продуктов</label>
  ...
  function syncPeerControl(control) {
    ...
    const value = Sprint.clamp(control.value, 0, maxAllowed);
  ...
  function submit(event) {
    ...
    encodedOutput.value = Sprint.encodeBlock(block);
